...
PORT="${PORT:-8134}"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"; [ -n "${SRVPID:-}" ] && kill "$SRVPID" 2>/dev/null; rm -f "$HERE/_ih25shot.html"' EXIT

cd "$HERE" || exit 1
sed "s#</body>#<script>window.IH25_SHOT_ZOOM=$ZOOM;window.IH25_SHOT_NAME='$NAME';window.IH25_SHOT_OVERLAY=${OVERLAY:-0};</script><script src=\"ih25_shot.js\"></script>\n</body>#" \
  ice_hockey_25d.html > _ih25shot.html
grep -q 'ih25_shot.js' _ih25shot.html || { echo "could not inject the probe (no </body>?)"; exit 1; }
...
timeout 180 google-chrome \
  ...
  --window-size="${SIZE/x/,}" \
  "http://localhost:$PORT/_ih25shot.html" >"$TMP/chrome.log" 2>&1 &
CHROME=$!
