#!/bin/bash
# Headless check of VS MODE and PLAYER SWITCHING in ice_hockey.html.
# Builds _vsprobe.html = ice_hockey.html + vsprobe.js, serves this folder,
...
trap 'rm -rf "$TMP"; [ -n "${SRVPID:-}" ] && kill "$SRVPID" 2>/dev/null; rm -f "$HERE/_vsprobe.html"' EXIT

cd "$HERE" || exit 1
sed 's#</body>#<script src="vsprobe.js"></script>\n</body>#' ice_hockey.html > _vsprobe.html
grep -q 'vsprobe.js' _vsprobe.html || { echo "could not inject the probe (no </body>?)"; exit 1; }
...
timeout 120 google-chrome \
  ...
  "http://localhost:$PORT/_vsprobe.html" >"$TMP/chrome.log" 2>&1 &
CHROME=$!
