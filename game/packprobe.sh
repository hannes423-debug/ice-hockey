...
# Builds _packprobe.html = ice_hockey.html + packprobe.js, serves this folder,
...
trap 'rm -rf "$TMP"; [ -n "${SRVPID:-}" ] && kill "$SRVPID" 2>/dev/null; rm -f "$HERE/_packprobe.html"' EXIT

cd "$HERE" || exit 1
sed 's#</body>#<script src="packprobe.js"></script>\n</body>#' ice_hockey.html > _packprobe.html
grep -q 'packprobe.js' _packprobe.html || { echo "could not inject the probe (no </body>?)"; exit 1; }
...
timeout 120 google-chrome \
  ...
  "http://localhost:$PORT/_packprobe.html" >"$TMP/chrome.log" 2>&1 &
CHROME=$!
