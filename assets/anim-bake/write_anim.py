...
import pickle,struct,base64,io as _io
TR=pickle.load(open('tracks.pkl','rb'))
    ...
    for bone in BONES:
        ...
        for c in anim['channels']:
            if c['target'].get('node')==node and c['target']['path']=='rotation':
                anim['samplers'][c['sampler']]={'input':ti,'output':oi,'interpolation':'LINEAR'}
                hit=True; nrep+=1
...
print("new ANIM_B64: %d bytes glb, %d chars base64 (was %d)"%(len(glb),len(b64),len(re.search(r'const ANIM_B64="([^"]+)"',S).group(1))))
open('anim_new.b64','w').write(b64)
# sanity: reparse
jj=json.loads(glb[20:20+struct.unpack('<I',glb[12:16])[0]])
print("reparsed ok: %d animations, %d accessors, %d bufferViews"%(len(jj['animations']),len(jj['accessors']),len(jj['bufferViews'])))
